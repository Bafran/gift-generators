...
class SimulationInstance:
    def __init__(self, headless=True, width=800, height=600, disk_radius=300):
        ...
        self.repulsion_strength = 100000
        self.velocity_threshold = 1.5 # steady state threshold
        self.fps = 60

        self.sim_bodies = []

    def create_polygon(self, vertices, position, scale_factor=1.0):
        # Single polygon
    ...
    def create_compound_polygon(self, vertices, position, scale_factor=1.0):
    ...
    def create_all_bodies(self, verticies_list):
        for i, (vertices, scale_factor, image_name) in enumerate(verticies_list):
            ...
            if isinstance(vertices[0][0], (list, tuple)):
                self.create_compound_polygon(vertices, position, scale_factor)
            else:
                self.create_polygon(vertices, position, scale_factor)
        ...
        for i, (body_a, shape_a) in enumerate(self.sim_bodies):
            for j, (body_b, shape_b) in enumerate(self.sim_bodies):
                if i != j:
                    ...
                    distance = math.sqrt(dx**2 + dy**2)
                    mass_factor = ((body_a.mass + body_b.mass) ** 2) / 4.0
...
if __name__ == "__main__":
    ...
    verticies_list = [
        (compute_polygon_parts(image_path + "/batteries.png", resolution=2.0), 2.0, "batteries"),
    ...
    run_pymunk_simulation(verticies_list)